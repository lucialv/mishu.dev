<template>
  <div class="mb-3">
    <h2
      class="font-bold text-xs text-zinc-700 dark:text-slate-300 leading-4 mb-2 uppercase"
    >
      Playing a game
    </h2>
    ...
    <div class="items-center flex">
      ...
      <div class="flex-auto overflow-hidden ml-2.5">
        <div
          class="font-semibold block whitespace-nowrap text-ellipsis overflow-hidden text-sm"
        >
          {{ data.name }}
        </div>
        <div
    ...
    <!-- buttons -->
    <div
      v-if="data.buttons?.length"
      class="flex flex-initial mt-3 flex-col flex-wrap justify-start items-stretch"
    >
      <button
...
const elapsed = useState<string | null>(
  "user-card-activity-elapsed",
  computeElapsed
);
...
function computeElapsed() {
  ...
  const seconds = (Math.floor(distance / secondAsMilliseconds) % 60)
    .toString()
    .padStart(2, "0");
  const minutes = (Math.floor(distance / minuteAsMilliseconds) % 60)
    .toString()
    .padStart(2, "0");
